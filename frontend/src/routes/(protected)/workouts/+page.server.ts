...
export const load: PageServerLoad = async ({ locals }) => {
	...
	try {
		const response = await fetch(`${GO_API_URL}/workouts/`, {
			// Ensure trailing slash matches Go route
			method: 'GET',
			headers: {
...
export const actions: Actions = {
	addWorkout: async ({ request, fetch, locals }) => {
		...
		if (!title || !description || !durationMinutesStr || !caloriesBurnedStr || !entriesString) {
			return fail(400, {
				...
				title,
				description,
				durationMinutes: durationMinutesStr,
				caloriesBurned: caloriesBurnedStr,
		...
		try {
			...
			for (const entry of entries) {
				if (
					!entry.exercise_name ||
					!entry.sets ||
					(entry.reps == null && entry.duration_seconds == null)
				) {
			...
			return fail(400, {
				formError: `Invalid entries data: ${(e as Error).message}`,
				title,
				description,
				durationMinutes: durationMinutesStr,
				caloriesBurned: caloriesBurnedStr,
		...
		const newWorkoutPayload = {
			...
			entries: entries.map((e, idx) => {
				const finalReps = e.reps != null && e.reps > 0 ? e.reps : null;
				const finalDuration =
					e.duration_seconds != null && e.duration_seconds > 0 ? e.duration_seconds : null;
		...
		console.log(
			'[Workouts Action] Sending payload to POST /workouts:',
			JSON.stringify(newWorkoutPayload, null, 2)
		);
		...
		try {
			...
			if (!response.ok) {
				// e.g., 201 Created is ok
				const errorResult = await response.json().catch(() => ({
					error: `API error (${response.status}) and failed to parse error response.`
				}));
				console.error(
					'[Workouts Action] API Error creating workout:',
					response.status,
					errorResult
				);
	...
	deleteWorkout: async ({ request, fetch, locals }) => {
		...
		try {
			...
			return fail(response.status, {
				message: errResult.error || errResult.message || 'Failed to delete.'
			});
		} catch {
