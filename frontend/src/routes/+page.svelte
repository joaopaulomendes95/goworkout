...
<main class="container">
	...
	{#if health}
		<div class="mb-6 rounded-lg bg-gray-800 p-4 shadow">
			<h2 class="mb-2 text-xl font-semibold text-blue-400">
                Backend Health Status
            </h2>
			{#if health.status === 'up'}
				<p class="text-green-400">Backend is healthy and running!</p>
				{#if health.message}<p class="text-xs text-gray-500">
                    Message: {health.message}</p>
                {/if}
			{:else}
	...
	<p class="mb-8 text-lg text-gray-300">
		This is a simple SvelteKit application to demonstrate the integration with a Go backend.
	</p>
</main>
